...
function initPortals(xRange:Number, yRange:Number):Void {
	for(var i:Number = 0; i < numOfPortals; i++) {
		...
		mc.nx = Stage.width - Math.ceil(Math.random() * xRange - 210);
...
function setHotSpot():Void {
	hotspot.onRollOver = function():Void {
		...
		if(Stage.width == 728) {
			...
			Tweener.addTween(cta_mc.arrow_mc, {_x: 59, time: .5, transition: "Strong.easeOut"});
		}
		//*/
	}
	hotspot.onRollOut = function():Void {
		...
		if(Stage.width == 728) {
			...
			Tweener.addTween(cta_mc.arrow_mc, {_x: 57, time: .5, transition: "Strong.easeOut"});
		}
		//*/
	}
}
