...
function randomizeLeaves():Void
{
	for(var i:Number = 0; i < 11; i++) { leaves_mc["leaf" + i].gotoAndPlay(Math.ceil(Math.random()*195)); }
};
...
function tweenLogosToBlack():Void
{
	var time:Number = 0.5;
	TweenLite.to(logo_mc.logoBlack_mc, time, { _alpha:100, ease:Expo.easeOut });
	TweenLite.to(logo_mc.logoWhite_mc, time, { _alpha:0, ease:Expo.easeOut });
};
...
function init():Void
{
	...
	{
		case "120x600" :
			logo_mc.logoBlack_mc._alpha = 0;
			logo_mc.logoWhite_mc._alpha = 100;
			break;
		case "160x600" :
			logo_mc.logoBlack_mc._alpha = 0;
			logo_mc.logoWhite_mc._alpha = 100;
...
function initBottomMenu():Void
{
	...
	bottomMenu_mc.onRollOver = function():Void { shimmer(); };
	bottomMenu_mc.onRelease = defaultClick;
	shimmer();
};
