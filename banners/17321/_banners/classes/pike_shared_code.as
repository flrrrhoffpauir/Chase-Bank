...
function flickerTheNeonOffToOn(time:Number):Void
{
	var adjustedTime:Number = time + 5000;
	trace("Next flicker in " + adjustedTime + " seconds.");
	clearInterval(_neonInt);
	_neonInt = setInterval(this, "flickOff", adjustedTime);
};

function flickOff():Void { sectionB_mc.gotoAndStop("off"); clearInterval(_neonInt); setTimeout(this, "flickOn", 100); };
function flickOn():Void { sectionB_mc.gotoAndStop("on"); flickerTheNeonOffToOn(Math.ceil(Math.random()*10000)); };
