...
class classes.tweening.TweenLite {
	...
	public function TweenLite($target:Object, $duration:Number, $vars:Object) {
		...
		if (this.forceActive || (this.vars.runBackwards == true && this.vars.renderOnStart != true)) {
			initTweenVals();
	...
	public static function executeAll():Void {
		var t:Number = currentTime = getTimer(), ml:Object = masterList, p:String, a:Array, i:Number;
		for (p in ml) {
			a = ml[p].tweens;
	...
	public static function killGarbage():Void {
		var ml:Object = masterList, p:String, tgt:Object, a:Array, i:Number;
		for (p in ml) {
			a = ml[p].tweens;
			for (i = a.length - 1; i > -1; i--) {
				if (a[i].gc) {
...
//---- PROXY FUNCTIONS -----------------------------------------------------------------------------------------------------------
	...
	public static function visibleProxy($o:Object, $time:Number):Void {
		...
		} else if ($time == 0 && t.vars.runBackwards == true) {
			t.target._visible = t.vars._visible;
