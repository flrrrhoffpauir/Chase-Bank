...
function alphaTween(mc:MovieClip, num:Number):Void { TweenLite.to(mc, TEXT_SPEED, { _alpha:num }); };
function xTween(mc:MovieClip, dlay:Number):Void { TweenLite.from(mc, TEXT_SPEED, { _x:-(STAGE_PADDING)-mc._width, ease:Expo.easeOut, delay:dlay }); };
function bgTween(yPos:Number):Void { TweenLite.to(bg, 4, { _y:yPos, ease:Linear.easeOut }); };
...
// -- Click Functions --------------------------- //
function defaultClick():Void { getURL(_root.clickTag, "_blank"); };
