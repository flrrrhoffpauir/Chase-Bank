...
class TweenLite {
	...
	public var target:Object; //Target object (usually a MovieClip)
		...
		if (this.target instanceof Array) {
			...
			for (i = 0; i < endArray.length; i++) {
				if (this.target[i] != endArray[i] && this.target[i] != undefined) {
					this.tweens.push({o:this.target, p:i.toString(), s:this.target[i], c:endArray[i] - this.target[i]}); //o: object, s:starting value, c:change in value, e: easing function
				}
			}
		} else {
			for (p in this.vars) {
	...
	private function addSubTween($proxy:Function, $target:Object, $props:Object, $info:Object):Void {
		...
		for (var p in $props) {
			if (typeof($props[p]) == "number") {
				this.tweens.push({o:$target, p:p, s:$target[p], c:$props[p] - $target[p], sub:sub}); //o:Object, p:Property, s:Starting value, c:Change in value;
			} else {
